...
- **Bloodlust safety** — green when the player can cast Bloodlust without a lockout and red while Sated, Exhaustion, Temporal Displacement, Fatigued, or Insanity is active. Shaman Bloodlust/Heroism, Mage Time Warp, Evoker Fury of the Aspects, Hunter Primal Rage, and Marksmanship Hunter Harrier's Cry are detected as castable lusts. Every class tracks received lust effects—including all drums generations—and its personal lockout: text mode shows a fast animated rainbow `BL ACTIVE | 00:40`, then returns to the lockout countdown when the buff ends. Bloodlust-capable characters finally show `BL | READY`; other characters show `BL | -`. Countdown times always use `mm:ss`, and text can be aligned left or right so its chosen edge stays fixed. Displaying the timer and icon is independent from enabling the selected music, so the music can play with every Bloodlust visual hidden. A 60-track music selector defaults to Dave Rodgers - Deja Vu and uses Blizzard's native aura-sound trigger so the selected song starts even when Bloodlust aura data is secret in combat. The bundled tracks are capped to the normal 40-second Bloodlust duration so playback ends with the buff; Play and Stop controls preview the selection in settings. Ferocity pets are detected through Command Pet's live Primal Rage override.
